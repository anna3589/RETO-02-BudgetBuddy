<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex flex-col items-center justify-end mt-4">
            <button type="submit"
                class="m-4 ms-3 inline-flex items-center px-4 py-2 bg-green-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-800 focus:bg-green-800 active:bg-green-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150">
                Iniciar Sesión
            </button>
            @if (Route::has('password.request'))
                <a class="underline text-sm mt-4 text-gray-600 hover:text-green-600 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"
        ...
        <div class="mt-6 text-center border-t pt-4">
            <p class="text-sm text-gray-600">
                ...
                <a href="{{ route('register') }}" class="font-bold text-green-700 hover:text-green-900">Regístrate
                    gratis</a>
            </p>
        </div>
    </form>
