...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function cambiarMoneda() {
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="currency-options">
                            ${monedas.map(moneda => `
                                <div class="currency-option" data-code="${moneda.codigo}">
                                    <div class="currency-symbol">${moneda.simbolo}</div>
                                    <div class="currency-info">
                                        <h4>${moneda.nombre}</h4>
                                        <div class="currency-details">
                                            <span class="currency-code">${moneda.codigo}</span>
                                            <span class="currency-country">${moneda.pais}</span>
                                        </div>
                                        ${moneda.descripcion ? `<div class="currency-extra-info">${moneda.descripcion}</div>` : ''}
                                    </div>
                                    <div class="currency-check">
                                        <i class="fas fa-check ${moneda.codigo === 'EUR' ? 'active' : ''}"></i>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
        ...
        const currentCurrencyOption = document.querySelector('.currency-option[data-code="EUR"]');
        ...
        if (saveCurrencyBtn) {
            saveCurrencyBtn.addEventListener('click', function() {
                const monedaSeleccionada = monedas.find(moneda => moneda.codigo === selectedCurrency);
                
                if (monedaSeleccionada) {
                    // Оновлюємо текст на сторінці
                    const settingsInfo = document.querySelector('.settings-item:nth-child(2) .settings-info p');
                    if (settingsInfo) {
                    ...
                    showNotification(`<i class="fas fa-money-bill-wave"></i> Moneda cambiada a ${monedaSeleccionada.simbolo} ${monedaSeleccionada.nombre}`, 'success');
        ...
        function adaptForMobile() {
            if (window.innerWidth <= 768) {
                ...
                let startY;
                currencyModal.addEventListener('touchstart', (e) => {
                    startY = e.touches[0].clientY;
                }, { passive: true });
                
                currencyModal.addEventListener('touchmove', (e) => {
                    if (!startY) return;
    ...
    function exportarDatos() {
        ...
        if (startExportBtn) {
            startExportBtn.addEventListener('click', function() {
                ...
                if (!exportProfile && !exportTransactions && !exportCards && !exportReports) {
                    showNotification('Por favor, selecciona al menos un tipo de dato para exportar.', 'info');
                    return;
                }
                ...
                startExportBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Procesando...';
                
                // Готуємо повідомлення про експорт
                let exportMessage = 'Exportando datos en formato ' + selectedFormat.toUpperCase() + '...\n\n';
                ...
                setTimeout(() => {
                    ...
                    let fileContent = '';
                    let fileName = `budgetbuddy_export_${new Date().toISOString().split('T')[0]}`;
                    let mimeType = 'text/plain';
                    
                    switch(selectedFormat) {
                        case 'csv':
                            fileContent = 'Fecha,Descripción,Categoría,Monto,Tipo\n';
                            fileContent += '2024-04-28,Compra supermercado,Alimentación,45.50,Gasto\n';
                            fileContent += '2024-04-27,Salario mensual,Ingresos,2500.00,Ingreso\n';
                            fileContent += '2024-04-26,Pago electricidad,Servicios,85.30,Gasto\n';
                            fileName += '.csv';
                            mimeType = 'text/csv';
                            break;
                        case 'pdf':
                            ...
                            fileContent += 'Fecha de exportación: ' + new Date().toLocaleDateString() + '\n';
                    ...
                    startExportBtn.innerHTML = '<i class="fas fa-download"></i> Iniciar exportación';
                    ...
                    showNotification(`<i class="fas fa-check-circle"></i> Datos exportados exitosamente como ${fileName}`, 'success');
        ...
        function adaptExportForMobile() {
            if (window.innerWidth <= 768) {
                ...
                let startY;
                exportModal.addEventListener('touchstart', (e) => {
                    startY = e.touches[0].clientY;
                }, { passive: true });
                
                exportModal.addEventListener('touchmove', (e) => {
                    if (!startY) return;
    ...
    function verSesiones() {
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="sessions-list">
                            ${sesiones.map(sesion => `
                                <div class="session-item ${sesion.esActual ? 'active' : ''}" data-id="${sesion.id}">
                                    ...
                                    <div class="session-info">
                                        ...
                                        <div class="session-details">
                                            <span class="session-detail">
                                                <i class="fas fa-globe"></i> ${sesion.navegador}
                                            </span>
                                            <span class="session-detail">
                                                <i class="fas fa-map-marker-alt"></i> ${sesion.ubicacion}
                                            </span>
                                            <span class="session-detail">
                                                <i class="fas fa-clock"></i> ${sesion.ultimaActividad}
                                            </span>
                                        </div>
                                        <div class="session-status ${sesion.esActual ? 'active' : 'inactive'}">
                                            <span class="status-dot ${sesion.esActual ? 'active' : 'inactive'}"></span>
                                            ${sesion.esActual ? 'Sesión actual' : 'Inactiva desde ' + sesion.ultimaActividad}
                                        </div>
                                    </div>
        ...
        if (closeAllSessionsBtn) {
            closeAllSessionsBtn.addEventListener('click', function() {
                if (confirm('¿Estás seguro de que quieres cerrar todas las sesiones excepto la actual?')) {
                    ...
                    closeAllSessionsBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Cerrando sesiones...';
                    closeAllSessionsBtn.disabled = true;
                    
                    setTimeout(() => {
                        // Симулюємо закриття сесій
                        const nonCurrentSessions = document.querySelectorAll('.session-item:not(.active)');
                        nonCurrentSessions.forEach(session => {
                            ...
                            const logoutBtn = session.querySelector('.session-action-btn[data-action="logout"]');
                            if (logoutBtn) {
                                logoutBtn.style.display = 'none';
                            }
                        });
                        
                        showNotification('<i class="fas fa-check-circle"></i> Todas las sesiones han sido cerradas correctamente.', 'success');
                        
                        // Повертаємо кнопку в початковий стан
                        closeAllSessionsBtn.innerHTML = btnText;
                        closeAllSessionsBtn.disabled = false;
                        closeAllSessionsBtn.innerHTML = '<i class="fas fa-check"></i> Sesiones cerradas';
                        closeAllSessionsBtn.disabled = true;
                        
                        setTimeout(() => {
                            closeAllSessionsBtn.innerHTML = '<i class="fas fa-sign-out-alt"></i> Cerrar todas las sesiones';
                            closeAllSessionsBtn.disabled = false;
                        }, 2000);
                    }, 1500);
                }
        ...
        if (refreshSessionsBtn) {
            refreshSessionsBtn.addEventListener('click', function() {
                ...
                refreshSessionsBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Actualizando...';
                refreshSessionsBtn.disabled = true;
                
                setTimeout(() => {
                    showNotification('<i class="fas fa-sync-alt"></i> Lista de sesiones actualizada.', 'success');
        ...
        actionButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (action === 'logout') {
                    if (confirm(`¿Estás seguro de que quieres cerrar la sesión en ${session.dispositivo}?`)) {
                        ...
                        setTimeout(() => {
                            ...
                            showNotification(`<i class="fas fa-check-circle"></i> Sesión en ${session.dispositivo} cerrada correctamente.`, 'success');
                        }, 1000);
                    }
                } else if (action === 'info') {
                    ...
                    const modalInfoHTML = `
                        <div class="modal" id="session-info-modal">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h3><i class="${session.icono}"></i> Información de sesión</h3>
                                ...
                                <div class="modal-body">
                                    <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 20px;">
                                        ...
                                        <div>
                                            <h4 style="margin: 0 0 5px 0; font-size: 18px;">${session.dispositivo}</h4>
                                            <div style="display: flex; align-items: center; gap: 10px;">
                                                <span class="status-dot ${session.esActual ? 'active' : 'inactive'}"></span>
                                                <span style="color: ${session.esActual ? '#00B934' : 'var(--text-muted)'}; font-size: 14px;">
                                                    ${session.esActual ? 'Sesión actual' : 'Sesión inactiva'}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div style="background: var(--gray-light); padding: 15px; border-radius: 10px; margin-bottom: 20px;">
                                        ...
                                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                                            <div>
                                                ...
                                                <div style="font-weight: 500;">${session.dispositivo}</div>
                                            </div>
                                            <div>
                                                <div style="font-size: 12px; color: var(--text-muted);">Navegador:</div>
                                                <div style="font-weight: 500;">${session.navegador}</div>
                                            </div>
                                            <div>
                                                <div style="font-size: 12px; color: var(--text-muted);">Sistema:</div>
                                                <div style="font-weight: 500;">${session.sistema}</div>
                                            </div>
                                            <div>
                                                <div style="font-size: 12px; color: var(--text-muted);">Ubicación:</div>
                                                <div style="font-weight: 500;">${session.ubicacion}</div>
                                            </div>
                                            <div>
                                                <div style="font-size: 12px; color: var(--text-muted);">IP:</div>
                                                <div style="font-weight: 500; font-family: monospace;">${session.ip}</div>
                                            </div>
                                            <div>
                                                <div style="font-size: 12px; color: var(--text-muted);">Fecha inicio:</div>
                                                <div style="font-weight: 500;">${session.fechaInicio}</div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div style="margin-bottom: 20px;">
                                        ...
                                        <ul style="margin: 0; padding-left: 20px;">
                                            <li>Inicio de sesión: ${session.fechaInicio}</li>
                                            <li>Última actividad: ${session.ultimaActividad}</li>
                                            ${session.esActual ? '<li>Sesión actualmente activa</li>' : '<li>Sesión inactiva</li>'}
                                        </ul>
                                    </div>
                    ...
                    const logoutThisSession = document.getElementById('logout-this-session');
                    ...
                    if (closeInfoModal) closeInfoModal.addEventListener('click', closeInfoModalFunc);
                    ...
                    if (logoutThisSession) {
                        logoutThisSession.addEventListener('click', function() {
                            if (confirm(`¿Estás seguro de que quieres cerrar la sesión en ${session.dispositivo}?`)) {
                                ...
                                const logoutBtn = sessionItem.querySelector('.session-action-btn[data-action="logout"]');
                                if (logoutBtn) {
                                    logoutBtn.style.display = 'none';
                                }
        ...
        if (helpSessionsBtn) {
            helpSessionsBtn.addEventListener('click', function() {
                showNotification('<i class="fas fa-question-circle"></i> Las sesiones muestran los dispositivos donde has iniciado sesión. Puedes cerrar sesiones no autorizadas aquí.', 'info');
            });
        }
        ...
        function adaptSessionsForMobile() {
            if (window.innerWidth <= 768) {
                // Додаємо свайп для закриття
                let startY;
                sessionsModal.addEventListener('touchstart', (e) => {
                    startY = e.touches[0].clientY;
                }, { passive: true });
                
                sessionsModal.addEventListener('touchmove', (e) => {
                    if (!startY) return;
    ...
    function initializeProfileData() {
        ...
        updateViewMode(initialData);
        updateEditMode(initialData);
        updateAvatar(initialData.firstName, initialData.lastName);
    }
    ...
    function updateViewMode(data) {
        ...
        // ОНОВЛЮЄМО ПОВНЕ ІМ'Я ПІД АВАТАРКОЮ
        if (profileFullNameView) {
            profileFullNameView.textContent = `${data.firstName} ${data.lastName}`;
        }
    }
    ...
    function saveProfileChanges() {
        const updatedData = {
            firstName: firstNameInput ? firstNameInput.value : 'John',
            lastName: lastNameInput ? lastNameInput.value : 'Doe',
            email: emailInput ? emailInput.value : 'john.doe@example.com',
            phone: phoneInput ? phoneInput.value : '+34 123 456 789'
        };
        
        initialFormValues = {...updatedData};
        updateViewMode(updatedData);
        updateAvatar(updatedData.firstName, updatedData.lastName);
        switchToViewMode();
        showNotification('<i class="fas fa-check-circle"></i> Los cambios se han guardado correctamente.', 'success');
    }
    ...
    // ===========================================
    function logoutUser() {
        if (confirm('¿Estás seguro de que quieres salir de tu cuenta?')) {
            showNotification('<i class="fas fa-sign-out-alt"></i> Cerrando sesión...', 'info');
            setTimeout(() => {
                showNotification('<i class="fas fa-check-circle"></i> Has salido de tu cuenta correctamente.', 'success');
                setTimeout(() => {
                    alert('En una aplicación real, serías redirigido a la página de inicio de sesión.');
                }, 1000);
            }, 1500);
        }
    }
    ...
    function initEventListeners() {
            ...
            changeAvatarBtn.addEventListener('click', function() {
                ...
                fileInput.addEventListener('change', function(e) {
                    if (e.target.files && e.target.files[0]) {
                        showNotification('<i class="fas fa-camera"></i> Foto de perfil actualizada correctamente.', 'success');
                    }
                });
        ...
        if (removeAvatarBtn) {
            removeAvatarBtn.addEventListener('click', function() {
                if (confirm('¿Estás seguro de que quieres eliminar tu foto de perfil?')) {
                    ...
                    showNotification('<i class="fas fa-trash"></i> Foto de perfil eliminada.', 'success');
                }
            });
        }
        ...
        if (passwordForm) {
            passwordForm.addEventListener('submit', function(e) {
                ...
                if (newPassword !== confirmPassword) {
                    showNotification('<i class="fas fa-exclamation-triangle"></i> Las contraseñas no coinciden.', 'info');
                    return;
                }
                
                if (newPassword.length < 8) {
                    showNotification('<i class="fas fa-exclamation-triangle"></i> La contraseña debe tener al menos 8 caracteres.', 'info');
                    return;
                }
                
                showNotification('<i class="fas fa-check-circle"></i> Contraseña cambiada correctamente.', 'success');
                closeModal(passwordModal);
                passwordForm.reset();
            });
        }
        ...
        if (twoFactorToggle) {
            twoFactorToggle.addEventListener('change', function() {
                showNotification(`<i class="fas fa-shield-alt"></i> Autenticación de dos factores ${this.checked ? 'activada' : 'desactivada'}.`, 'info');
            });
        }
        
        if (emailNotificationsToggle) {
            emailNotificationsToggle.addEventListener('change', function() {
                showNotification(`<i class="fas fa-envelope"></i> Notificaciones por email ${this.checked ? 'activadas' : 'desactivadas'}.`, 'info');
            });
        }
        
        if (pushNotificationsToggle) {
            pushNotificationsToggle.addEventListener('change', function() {
                showNotification(`<i class="fas fa-bell"></i> Notificaciones push ${this.checked ? 'activadas' : 'desactivadas'}.`, 'info');
            });
        }
        
        if (spendingAlertsToggle) {
            spendingAlertsToggle.addEventListener('change', function() {
                showNotification(`<i class="fas fa-exclamation-circle"></i> Alertas de gastos ${this.checked ? 'activadas' : 'desactivadas'}.`, 'info');
            });
        }
