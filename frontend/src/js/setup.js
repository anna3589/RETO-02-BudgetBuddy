			...
			try {
				...
				const accRes = await fetch("/api/accounts", {
					...
					headers: {
						"Content-Type": "application/json",
						Accept: "application/json",
						"X-XSRF-TOKEN": xsrfToken, // <--- NUEVO: La llave de seguridad
					},
					credentials: "same-origin", // <--- VITAL: Envía tu identidad (Cookie de sesión)
					body: JSON.stringify(accountData),
				});

				if (!accRes.ok) {
					// Intentamos leer el mensaje de error que manda Laravel
					const errorData = await accRes.json();
					console.error("ERROR DETALLADO:", errorData); // Míralo en la consola (F12)

					// Mostramos el mensaje real en la alerta
					throw new Error(
						errorData.message || "Error desconocido (" + accRes.status + ")"
					);
				}
				...
				if (document.getElementById("has_envelope").checked) {
					...
					const iconClass = selectedIconDiv
